...
import settings_manager.loading
from settings_manager import loading, config_items
import os
...
class TestGetConfigItem(object):
    ...
    def test_no_type_specified(self):
        result = isinstance(
            settings_manager.loading.get_config_item("string"),
            config_items.DefaultConfigItem
        )
        assert result, "Type is DefaultConfigType"

    def test_type_specified(self):
        result = isinstance(
            settings_manager.loading.get_config_item({"_type": "settings_manager.config_items.ValueLookupConfigItem"}),
            config_items.ValueLookupConfigItem
        )
